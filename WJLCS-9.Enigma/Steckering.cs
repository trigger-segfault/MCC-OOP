	...
	public class Steckering : IReadOnlyList<int> {
		...
		public Steckering(int[] steckering) {
			...
			if (steckering.Length == 0)
				throw new ArgumentException($"{nameof(steckering)}.{nameof(steckering.Length)} is zero!", nameof(steckering));
			...
			for (int inputIndex = 0; inputIndex < steckering.Length; inputIndex++) {
				...
				if (usedIndexes[outputIndex])
					throw new ArgumentException($"{nameof(steckering)} remap {outputIndex} has already been used!", nameof(steckering));
		...
		/// <returns>The index of the remap index.</returns>
